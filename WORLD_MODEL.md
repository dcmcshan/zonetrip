...
- `charter.md` is the read-only constitution supplied to the LLM
- `model.md` is the surviving derived world model, not source material
...
Any implementation that needs temporary speech-to-text must treat it as an internal processing buffer and destroy it after the derived update is complete. The update cycle is: read `charter.md`, read current `model.md`, process temporary STT, generate a complete replacement `model.md`, then discard request-local transcript material.
